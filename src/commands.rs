...
pub enum Commands {
    ...
    Update {
        ...
        /// New answer text (optional)
        #[arg(long)]
        answer: Option<String>,

        /// New guidance/hint text (optional)
        #[arg(long)]
        guidance: Option<String>,
    },

    /// Batch update multiple flashcards from a CSV file
    ///
    /// # Usage:
    /// ```
    /// words batch-update username deck_name path [--dry-run]
    /// ```
    /// Format: id ~ question ~ answer ~ guidance
    UpdateMany {
        /// Username of the account
        username: String,

        /// Name of the deck to update
        deck_name: String,

        /// Path to CSV file containing updates
        path: String,

        /// Run in dry-run mode to check for errors without making changes
        #[arg(long)]
        dry_run: bool,
    },
...
pub fn handle_command(cmd: &Commands, db: &mut DatabaseManager) -> Result<()> {
    match cmd {
        ...
        Commands::NewCard {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                db.new_card_with_state(*deck_id, user_id, &card)?;
        ...
        Commands::Review {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                let (cards, total_cards, queue_info) = if algorithm == "fsrs" {
                    ...
                    let (new_cards, learning_cards, review_cards) =
                        db.get_due_cards_by_queue(*deck_id)?;
                    ...
                    ordered_cards
                        .extend(learning_cards.into_iter().take(learning_cards_to_include));

                    (ordered_cards, total, queue_info)
                } else {
                    // Use standard card fetching for SM2
                    let cards = db.get_due_cards(*deck_id)?;
                    let total = cards.len();
                ...
                println!(
                    "\n📚 Starting review session - {} cards due (batch size: {})",
                    total_cards, batch
                );
                ...
                while card_iter.peek().is_some() {
                    for (i, (card_id, card)) in card_iter.by_ref().take(*batch).enumerate() {
                        ...
                        db.update_card_state(user_id, card_id, rating)?;

                        // Get the updated card data for the summary
                        let updated_card = db.get_card(card_id)?;
                        ...
                        println!(
                            ...
                            next_review_date, updated_card.interval, interval_change
                        );
                    }
                ...
                if !review_summary.is_empty() {
                    ...
                    for (question, rating, old_interval, new_interval, _, next_review, card_id) in
                        &review_summary
                    {
                        ...
                        if algo_name == "fsrs" {
                            if let Ok((difficulty, stability, retrievability)) =
                                db.get_fsrs_fields(*card_id)
                            {
                    ...
                    println!(
                        "\nSession ID: {} - Use 'history' command to view past reviews",
                        session_id
                    );
                }
            } else {
        ...
        Commands::Stats {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                println!(
                    "\n📊 Statistics for '{}' (Total: {}, Due: {})",
                    deck_name, total_cards, due_cards
                );
        ...
        Commands::NewMany {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                let (imported, skipped, errors) = db.import_cards_from_csv_with_algorithm(
                    user_id,
                    deck,
                    path,
                    algorithm.as_deref(),
        ...
        Commands::Find { query } => {
            let matches = db.search_card_globally(query)?;
            ...
            for (i, (user_id, username, deck_id, deck_name, card, algorithm)) in
        ...
        Commands::ToFsrs { username, deck_id } => {
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                let (deck_name, total_cards, _) = match db.get_deck_details(user_id, *deck_id) {
                    ...
                    Err(_) => {
                        println!(
                            "Deck with ID {} not found or doesn't belong to '{}'",
                            deck_id, username
                        );
                        return Ok(());
                    }
                ...
                println!(
                    "Deck '{}' successfully converted to FSRS algorithm",
                    deck_name
                );
                ...
                match type_.as_str() {
                    ...
                    "efficiency" => {
                        ...
                        let efficiency_analysis =
                            db.analyze_learning_efficiency(user_id, *deck_id)?;
        ...
        Commands::Simulate { question } => {
            ...
            let card_question = question
                .clone()
                .unwrap_or_else(|| "Example Card".to_string());
        ...
        Commands::Graduate { username, deck_id } => {
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                if graduated_count > 0 {
                    println!(
                        "Successfully graduated {} cards from learning to review stage",
                        graduated_count
                    );
                    println!("These cards had consistently high ratings (4-5) but were stuck in the learning phase.");
                    println!(
                        "They will now appear in the review queue instead of the learning queue."
                    );
                    ...
                    db.log_operation(
                        ...
                        *deck_id,
                        Some(&format!(
                            "Graduated {} cards from learning to review",
                            graduated_count
                        )),
                    )?;
                } else {
                    println!(
                        "No cards in deck '{}' currently qualify for graduation.",
                        deck_name
                    );
        ...
        Commands::Update {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                ...
                db.update_card_content_with_verification(
                    user_id,
                    deck_name,
                    *card_id,
                    question.as_deref(),
                ...
                println!(
                    "Card {} in deck '{}' updated successfully",
                    card_id, deck_name
                );
            } else {
                println!("User '{}' not found!", username);
        ...
        Commands::UpdateMany {
            ...
            if let Some(user_id) = db.authenticate_user(username)? {
                let (updated, skipped, errors) =
                    db.update_cards_content_from_csv(user_id, deck_name, path)?;

                println!("Batch update completed:");
                println!("- Successfully updated: {} flashcards", updated);
