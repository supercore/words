...
impl DatabaseManager {
    ...
    fn initialize_schema(conn: &Connection) -> Result<()> {
        ...
        if !has_old_interval {
            ...
            conn.execute("ALTER TABLE reviews ADD COLUMN old_interval INTEGER", [])
    ...
    pub fn new_card_with_state(&self, deck_id: i64, user_id: i64, card: &Flashcard) -> Result<i64> {
        let timestamp = get_current_timestamp()?;
        ...
        while self.is_card_exists(deck_id, &unique_question)? {
    ...
    pub fn update_card_state(&self, user_id: i64, card_id: i64, rating: i32) -> Result<()> {
        let timestamp = get_current_timestamp()?;

        // Get current flashcard state before update (for history)
        let current_card = self.get_card(card_id)?;
        ...
        let row = stmt.query_row(params![card_id], |row| {
            Ok((
                ...
                row.get::<_, String>(10)
                    .unwrap_or_else(|_| "sm2".to_string()), // algorithm
        ...
        if algo_name == "fsrs" {
            ...
            let fsrs_card =
                crate::fsrs::flashcard_to_fsrs(&card, difficulty, stability, retrievability);
    ...
    fn is_card_exists(&self, deck_id: i64, question: &str) -> Result<bool> {
        let mut stmt = self
            .conn
    ...
    pub fn get_due_cards(&self, deck_id: i64) -> Result<Vec<(i64, Flashcard)>> {
        let now = get_current_timestamp()?;

        // Get algorithm used by deck
        let algo_name: String = self
            .conn
            .query_row(
    ...
    pub fn analyze_review_order(&self, deck_id: i64) -> Result<String> {
        ...
        if algorithm == "fsrs" {
            ...
            output.push_str(&format!(
                "FSRS Review Analysis: {} cards due\n",
                total_cards
            ));
            output.push_str(&format!(
                "- Learning cards: {} (should be reviewed first)\n",
                learning_cards.len()
            ));
            output.push_str(&format!(
            ...
            output.push_str(&format!(
                "- Review cards: {} (maintain long-term retention)\n",
                review_cards.len()
            ));
            ...
            if !learning_cards.is_empty() {
                ...
                for i in 0..display_limit {
                    ...
                    // Get FSRS fields
                    let (_, _stability, retrievability) =
                        self.get_fsrs_fields(*card_id).unwrap_or((5.0, 0.0, 1.0));
            ...
            let mut output = String::new();
            output.push_str(&format!(
                "Due Cards Analysis: {} cards due for review\n",
                total_cards
            ));
            ...
            if !cards_with_priority.is_empty() {
                ...
                for i in 0..display_limit {
                    let (id, question, repetitions, _, _, overdue_seconds, _, priority) =
                        &cards_with_priority[i];
            ...
            if !ordering_issues.is_empty() {
                ...
                output.push_str(&format!(
                    "- {} cards ({:.1}%) would benefit from reordering\n",
                    ordering_issues.len(),
    ...
    pub fn create_deck_with_algorithm(
        &self,
        user_id: i64,
        name: &str,
        algorithm: &str,
        ...
        match existing_deck_id {
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => {
                ...
                self.log_operation(
                    ...
                    "DECK",
                    deck_id,
                    Some(&format!("{} (algorithm: {})", name, algorithm)),
                )?;
                Ok(deck_id)
            }
            Err(e) => Err(e.into()),
    ...
    pub fn analyze_fsrs_recommendations(&self, user_id: i64) -> Result<String> {
        ...
        for deck_result in deck_iter {
            ...
            let recommendation = if total_cards > 100 || review_count > 500 {
                format!(
                    "Highly Recommended ⭐⭐⭐ (convert-to-fsrs --deck-id {})",
                    deck_id
                )
    ...
    pub fn get_performance_trend(&self, user_id: i64, deck_id: i64) -> Result<String> {
        ...
        if algorithm == "fsrs" {
            ...
            output.push_str(&format!(
                "Average memory retention rate: {:.1}%\n",
    ...
    pub fn get_card(&self, card_id: i64) -> Result<Flashcard> {
        let mut stmt = self.conn.prepare(
    ...
    pub fn get_review_history(
        ...
        deck_id: i64,
        limit: usize,
    ) -> Result<Vec<(u64, String, i32, u32, u32, u64, String, i64)>> {
    ...
    fn add_old_interval_column_if_needed(&self) -> Result<()> {
        ...
        if !has_old_interval {
            println!("Upgrading database: Adding 'old_interval' column to 'reviews' table...");
            self.conn
                .execute("ALTER TABLE reviews ADD COLUMN old_interval INTEGER", [])
                .context("Failed to add old_interval column to reviews table")?;
        }
    ...
    pub fn get_interval_statistics(&self, deck_id: i64) -> Result<Vec<(String, i64, f64)>> {
        ...
        for (min, max, label) in ranges.iter() {
            ...
            let count: i64 = self.conn.query_row(
                &format!(
                    "SELECT COUNT(*) FROM flashcards WHERE deck_id = ? AND {}",
                    where_clause
                ),
                params![deck_id],
                |row| row.get(0),
    ...
    pub fn analyze_learning_efficiency(&self, user_id: i64, deck_id: i64) -> Result<String> {
        ...
        efficiency_table.add_row(row![
            "Cards learned per study hour",
            format!("{:.1}", cards_per_hour)
        ]);
        efficiency_table.add_row(row![
            "Total study time",
            format!("{:.1} hours", total_time / 60.0)
        ]);
    ...
    pub fn balance_review_load(&self, user_id: i64, days_ahead: u32) -> Result<String> {
        ...
        let mut output = String::new();
        output.push_str(&format!(
            "Review Load Analysis for Next {} Days\n\n",
            days_ahead
        ));
        ...
        if has_imbalance {
            ...
            output.push_str(&format!(
                "Ideal cards per day: {} (total {} cards over {} days)\n",
    ...
    pub fn get_due_cards_by_queue(
        ...
        if algorithm != "fsrs" {
            ...
            let all_cards = self.get_due_cards(deck_id)?;
        ...
        let process_rows = |stmt: &mut rusqlite::Statement,
                            params: &[&dyn rusqlite::ToSql],
                            queue: &mut Vec<(i64, Flashcard)>|
         -> Result<()> {
    ...
    pub fn get_queue_distribution(&self, deck_id: i64) -> Result<String> {
        ...
        if algorithm == "fsrs" {
            ...
            let due_cards = self.get_due_cards(deck_id)?;
            output.push_str(&format!(
                "\n\nDue cards: {}/{} ({:.1}%)",
                due_cards.len(),
                total_cards,
    ...
    /// Update flashcard content while preserving scheduling data
    pub fn update_card_content(
        &self,
        card_id: i64,
        new_question: Option<&str>,
        new_answer: Option<&str>,
        new_guidance: Option<&str>,
    ) -> Result<()> {
        // Check if the card exists first
        let card_exists = self.conn.query_row(
            "SELECT COUNT(*) FROM flashcards WHERE id = ?",
            params![card_id],
            |row| row.get::<_, i64>(0),
        )?;
        ...
        if updates.is_empty() {
            return Err(anyhow::anyhow!("No valid fields to update provided"));
        }

        // Build the full query
        let query = format!("UPDATE flashcards SET {} WHERE id = ?", updates.join(", "));

        // Add card_id parameter
        ...
        // Execute the query
        let affected = self.conn.execute(&query, &param_refs[..])?;

        if affected == 0 {
            // This shouldn't happen since we checked existence earlier, but just in case
            return Err(anyhow::anyhow!(
                "Card with ID {} exists but no changes were made",
                card_id
            ));
        }
    ...
    /// Update a card by ID with provided content while verifying ownership
    pub fn update_card_content_with_verification(
        &self,
        user_id: i64,
        deck_name: &str,
        card_id: i64,
        ...
        // Get deck ID from name and verify ownership
        let deck_id = self.get_deck_id(user_id, deck_name)?;

        // Verify the card exists and belongs to the specified deck
        let card_exists = self.conn.query_row(
            "SELECT COUNT(*) FROM flashcards WHERE id = ? AND deck_id = ?",
        ...
        self.update_card_content(card_id, question, answer, guidance)?;
        ...
        self.log_operation(
            ...
            "FLASHCARD",
            card_id,
            Some("Updated card content"),
        )?;

        Ok(())
    }

    pub fn update_cards_content_from_csv(
        &self,
        user_id: i64,
        deck_name: &str,
        csv_path: &str,
        ...
        // Get deck ID and verify ownership
        let deck_id = self.get_deck_id(user_id, deck_name)?;

        let file = File::open(csv_path)?;
        let reader = BufReader::new(file);

        let mut cards_to_updte = Vec::new();
        ...
        for (line_num, line) in reader.lines().enumerate() {
            let line = match line {
                Ok(r) => r,
                Err(e) => {
                    let error = format!("Line {}: {}", line_num + 1, e);
            ...
            if parts.len() < 3 {
                let error = format!(
                    "Line {}: Invalid format (expected 'question ~ answer ~ guidance)",
                    line_num + 1
                );
            ...
            cards_to_updte.push(card);
        }
        let card_refs: Vec<&Flashcard> = cards_to_updte.iter().collect();
        match self.update_cards_content(deck_id, user_id, card_refs) {
            Ok(_) => {
                ...
                self.log_operation(
                    ...
                    "FLASHCARDS",
                    deck_id,
                    Some(&format!("Update {} cards from CSV", cards_to_updte.len())),
                )?;
                Ok((cards_to_updte.len(), invalid_lines.len(), error_messages))
            }
            Err(e) => {
    ...
    /// Batch update multiple flashcards
    ///
    /// Updates cards by question in the specified deck.
    /// This is the update counterpart to batch_add_flashcards.
    pub fn update_cards_content(
        &self,
        deck_id: i64,
        user_id: i64,
        cards: Vec<&Flashcard>,
        ...
        if stmt
            .query_row(params![deck_id, user_id], |_| Ok(()))
            .is_err()
        {
            return Err(anyhow::anyhow!(
        ...
        // Begin transaction to process all updates as a single atomic unit
        ...
        let mut lookup_stmt = self
            .conn
            .prepare("SELECT id FROM flashcards WHERE deck_id = ? AND question LIKE ?")?;

        let mut update_stmt = self.conn.prepare(
            "UPDATE flashcards SET answer = ?, guidance = ? WHERE id = ? AND deck_id = ?",
        )?;

        for card in cards {
            // Look up card ID by question (using exact match or with added suffix)
            let search_pattern = format!("{}%", card.question);
            let card_id_result =
                lookup_stmt.query_row(params![deck_id, search_pattern], |row| row.get::<_, i64>(0));

            match card_id_result {
                Ok(card_id) => {
                    // Card found, update it
                    update_stmt.execute(params![&card.answer, &card.guidance, card_id, deck_id])?;
                    updated_count += 1;
                }
                Err(_) => {
                    // Card not found by question
                    errors.push(format!("No card found with question: {}", card.question));
                }
            }
        }

        if updated_count == 0 && !errors.is_empty() {
            // If no cards were updated and there were errors, roll back
            self.conn.execute_batch("ROLLBACK;")?;
            return Err(anyhow::anyhow!(
                "No cards were updated. First error: {}",
                errors[0]
            ));
        }
    ...
    pub fn import_cards_from_csv(
        &self,
        user_id: i64,
        deck_id: i64,
        csv_path: &str,
    ) -> Result<(usize, usize, Vec<String>)> {
        // Return imported count, skipped count, and errors
        ...
        match self.new_cards_with_state(deck_id, user_id, card_refs) {
            Ok(_) => {
                println!(
    ...
    pub fn import_cards_from_csv_with_algorithm(
        &self,
        user_id: i64,
        deck_name: &str,
        csv_path: &str,
        ...
        if algorithm_name != "sm2" && algorithm_name != "fsrs" {
            return Err(anyhow::anyhow!(
                "Invalid algorithm: {}. Valid options are 'sm2' or 'fsrs'",
                algorithm_name
            ));
        }
        ...
        let deck_id = match self.get_deck_id(user_id, deck_name) {
            Ok(id) => {
                ...
                if algorithm.is_some() {
                    ...
                    if current_algo != algorithm_name {
                        println!(
                            "Warning: Not changing existing deck algorithm from {} to {}",
            ...
            Err(_) => {
                ...
                println!(
                    "Creating new deck '{}' with {} algorithm",
                    deck_name, algorithm_name
                );
        ...
        self.import_cards_from_csv(user_id, deck_id, csv_path)
    }
