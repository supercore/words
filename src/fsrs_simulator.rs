use crate::fsrs::{flashcard_to_fsrs, FsrsCard, FsrsParameters, MemoryState};
...
pub fn simulate_fsrs_scheduling(question: &str) -> String {
    ...
    for rating in 1..=5 {
        ...
        let stability_change = if second_card.state.stability > first_stability {
    ...
    for rating in 1..=5 {
        ...
        let stability_change = if card_copy.state.stability > mature_card.state.stability {
            format!(
                "+{:.1}",
                card_copy.state.stability - mature_card.state.stability
            )
        } else if card_copy.state.stability < mature_card.state.stability {
            format!(
                "-{:.1}",
                mature_card.state.stability - card_copy.state.stability
            )
        } else {
            "=".to_string()
        };
    ...
    output.push_str(&table.to_string());
    output.push_str("\n");
    output.push_str("Memory Retention Visualization for Mature Card:\n");
    output.push_str(&add_forgetting_curve(&mature_card));
    output.push_str("\n\n");
    ...
    output.push_str("==========================================================\n");
    output.push_str(
        "This simulates how a card progresses through 10 review cycles with consistent ratings\n\n",
    );
    ...
    output.push_str("\n");
    output.push_str("Memory Retention Curve after 10 perfect reviews:\n");
    output.push_str(&add_forgetting_curve(&perfect_card));
    output.push_str("\n\n");
    ...
    output.push_str(
        "The algorithm is based on a memory model that predicts memory decay over time.\n",
    );
    output
}
...
// Add a visual representation of the forgetting curve
pub fn add_forgetting_curve(card: &FsrsCard) -> String {
    let mut output = String::new();
    output.push_str("\nMemory Retention Curve:\n");

    let stability = card.state.stability;
    let mut table = Table::new();
    table.add_row(row!["Days", "Retention", "Visual"]);

    // Show retention at different time points
    for days in [0, 1, 3, 7, 14, 30, 60, 90, 180] {
        let days = days as f64;
        if days > stability * 5.0 {
            continue; // Skip days too far in the future
        }

        let retention = (-days / stability).exp();
        let visual_bar = create_visual_bar(retention);

        table.add_row(row![
            format!("Day {}", days as i32),
...
fn create_visual_bar(value: f64) -> String {
    let full_width = 20;
    let filled = (value * full_width as f64).round() as usize;
    let empty = full_width - filled;
