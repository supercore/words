...
impl FsrsCard {
    ...
    pub fn update(&mut self, rating: u32, params: &FsrsParameters) {
        ...
        let new_stability = if rating < 3 {
            ...
            if self.state.stability <= 0.0 {
                ...
                self.state.stability
                    * (1.0 + rating_weight * s_factor * stability_boost * self.state.retrievability)
            }
        };
        ...
        let interval = if rating < 3 {
            ...
            (days.max(min_interval as f64))
                .min(params.maximum_interval as f64)
                .ceil() as u32
        };
...
impl FsrsAlgorithm {
    ...
    pub fn _calibrate_parameters(
        &self,
        db: &DatabaseManager,
        user_id: i64,
    ) -> Result<FsrsParameters, anyhow::Error> {
        ...
        let new_params = FsrsParameters {
            request_retention: (self.params.request_retention + retention_adjustment)
                .max(0.8)
                .min(0.95),
            w: [
...
impl crate::algorithm::SpacedRepetitionAlgorithm for FsrsAlgorithm {
    ...
    fn convert(&self, card: &Flashcard) -> (bool, f64, f64, f64) {
        ...
        (
            true,
            mem_state.difficulty,
            mem_state.stability,
            mem_state.retrievability,
        )
    }
...
pub fn graduate_well_known_cards(
    db: &DatabaseManager,
    deck_id: i64,
